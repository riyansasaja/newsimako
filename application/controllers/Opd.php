...
class Opd extends CI_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        is_logged_in();
        $this->load->model('Modelopd', 'model');
    }
    public function riwayat()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $cek =  $this->model->tambahriwayatrisiko();
            if ($cek) {
    ...
    {
        $id_user = $this->session->userdata('id');
        $data = $this->model->tambahProgram($id_user);
        echo json_encode($data);
    }
    ...
    public function getprogram()
    {
        ...
        $data = $this->db->get_where('tb_program', ['id_user' => $id_user, 'tahun_anggaran' => $tahun_anggaran])->result();
        $json = [
            'data' => $data
        ];
    ...
    public function get_realisasi_program()
    {
        $id_user = $this->session->userdata('id');
        $tahun_anggaran = $this->session->userdata('tahun_anggaran');
        $data = $this->db->get_where('tb_program', ['id_user' => $id_user, 'realisasi' => 1, 'tahun_anggaran' => $tahun_anggaran])->result();
        $json = [
            'data' => $data
        ];
        echo json_encode($json);
    ...
    public function getkegiatan()
    {
        ...
        $data = $this->db->get_where('tb_tujuan_kegiatan', ['id_user' => $id_user, 'tahun_anggaran' => $tahun_anggaran])->result();
        $json = [
            'data' => $data
        ];
        echo json_encode($json);
    ...
    {
        $id_user = $this->session->userdata('id');
        $data = $this->model->tambah($id_user);
        echo json_encode($data);
    }
    ...
    public function editlistriwayat($id)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $cek =  $this->model->updateriwayat($id);
            if ($cek) {
